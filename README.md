...
Личный кабинет партнёра + интеграция с amoCRM через Albato. Партнёр получает 10% с любых оплаченных сделок приглашённого клиента (пожизненно).
...
4. **Назначение:** «Webhook» / «HTTP запрос».
   ...
   - **Тело:**
     ...
     ```
     (точные имена переменных зависят от Albato — выбирай в их визуальном маппере поля «ID сделки», «Бюджет сделки», «ID контакта», «REFERER (контакт)»)
